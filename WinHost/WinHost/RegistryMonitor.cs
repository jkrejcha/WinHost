/// This is a modified version of RegistryMonitor.cs, available here:
...
namespace WinHost
{
	...
	/// </code>
	/// </example>
	sealed class RegistryMonitor : IDisposable
	{
		#region P/Invoke
		...
		private RegChangeNotifyFilter _regFilter = RegChangeNotifyFilter.Key | RegChangeNotifyFilter.Value;
		...
		{
			if (name == null || name.Length == 0) throw new ArgumentNullException();
		...
		public RegChangeNotifyFilter RegChangeNotifyFilter
		{
			...
			set
			{
				lock (_threadLock)
				{
					if (IsMonitoring) throw new InvalidOperationException();
